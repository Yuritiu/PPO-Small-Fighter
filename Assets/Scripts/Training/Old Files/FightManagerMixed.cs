...
public class FightManagerMixed : MonoBehaviour
{
    ...
    public void StartMatchStandalone()
    {
        ...
        {
            var found = FindObjectsOfType<NewFighter>(true); // include inactive
            if (found.Length >= 2)
            {
                fighters = new NewFighter[2] { found[0], found[1] };
            }
            else
            {
