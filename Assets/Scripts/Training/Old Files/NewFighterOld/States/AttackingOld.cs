...
public class AttackingOld : FighterStateOld
{
    private Vector3 acceleration;

    public AttackingOld(NewFighterOld fighter) : base(fighter)
    {
    }
    ...
    {        
        if (fighter.currentAction == null || fighter.currentFrame >= fighter.currentAction.numberOfFrames)
        {
            if (!fighter.onGround)
            {
                fighter.SwitchState(new FallingOld(fighter));
            }
            else
            {
        ...
        else
        {
            ...
            if (fighter.currentAction.playWhiffSound && fighter.currentFrame == fighter.currentAction.whiffPlayFrame)
            {
                ...
                {   
                    ...
                    {
                        FightManagerOld.instance.PlaySound(SoundType.Whiff, fighter.audioSource);
                    }
                }
            }
