...
public class NewFighterOld : MonoBehaviour
{
    public const int BackLayer = 7;
    public const int FrontLayer = 8;

    [HideInInspector] public UnityEvent<NewFighterOld, NewFighterOld> BreakThrow;
    [HideInInspector] public UnityEvent<NewFighterOld> TookDamage;
    private bool paused;
    ...
    public NewFighterOld throwOpponent { get; set; }
    ...
    private AIInputProviderOld aiInput;
    ...
    public FighterStateOld currentState;
    ...
    private void Awake()
    {
        ...
        aiInput = GetComponent<AIInputProviderOld>();
    ...
    private void Start()
    {
        SwitchState(new WalkingOld(this));
    }

    private void Update()
    {
        if (useAIInput && aiInput != null)
        {
            currentInput = aiInput.GetInput();
        }
        else
        {
        ...
        if (hitThisFrame != null)
        {
            if (hitThisFrame.action.type == ActionData.Type.Grab)
            {
                if (actionHasHit && currentAction != null && currentAction.type == ActionData.Type.Grab)
                {
                    ...
                    FightManagerOld.instance.OnBreakThrow(this, hitThisFrame.hitbox.transform.parent.GetComponent<NewFighterOld>());
                }
                else if (!actionHasHit || (currentAction != null && currentAction.type < ActionData.Type.Grab))
                {
                    if (currentState is WalkingOld || currentState is AttackingOld)
                        if (FightManagerOld.instance != null)
                        {
                            FightManagerOld.instance.ThrowFighter(this, hitThisFrame.hitbox.transform.parent.GetComponent<NewFighterOld>(), hitThisFrame.action);
                        }
                }
            }
            else if (!actionHasHit || (currentAction != null && currentAction.type <= hitThisFrame.action.type))
            {
                if (FightManagerOld.instance != null)
                {
                    FightManagerOld.instance.OnFighterHit(this, hitThisFrame, blocking);
                }
    ...
    public void SwitchState(FighterStateOld nextState)
    {
    ...
    private void SwitchAction(List<ActionData> potentialActions)
    {
        ...
        {
            if ((currentState is WalkingOld && !nextAction.airOkay) || (currentState is JumpingOld && velocity.y <= 10f && nextAction.airOkay))
            {
                if (currentAction == null || currentAction.alwaysCancelable)
                {
                    if (nextAction.projectiles.Length == 0 || canSpawnProjectile)
                    {
                        ...
                        SwitchState(new AttackingOld(this));
                        return;
                    }
                }
            }
            else if (currentState is AttackingOld && currentAction != null)
            {
    ...
    public void GetHit(ActionData action, int hitStunFrames, int blockStunFrames)
    {
        ...
        if (!blocking)
        {
            ...
            if (onGround)
            {
                if (action.knockdown)
                {
                    ...
                    SwitchState(new KnockdownOld(this));
                }
                else
                {
                    ...
                    SwitchState(new StunnedOld(this, hitStunFrames));
                }
            }
            else
            {
        ...
        else
        {
            ...
            FightManagerOld.instance.PlaySound(SoundType.Block, audioSource);
            SwitchState(new StunnedOld(this, blockStunFrames));
        }
    }
    ...
    public void SpawnProjectile(ProjectileData data)
    {
        ...
        offset.x += (IsOnLeftSide ? 0.25f : -0.25f);
        GameObject go = Instantiate(data.projectilePrefab, transform.position + offset, rotation);

        Projectile projectile = go.GetComponentInChildren<Projectile>(true);
        if (projectile == null)
        {
            Debug.LogError($"Projectile prefab '{data.projectilePrefab.name}' has no Projectile component (root or children).");
            canSpawnProjectile = true;
            return;
        }
        projectile.ProjectileDestroyed.AddListener(OnProjectileDespawned);
        projectile.InitOld(this);
    }
    ...
    public void OnHitboxCollides(Collider2D col)
    {
        ...
        {
            NewFighterOld owner = col.transform.parent.GetComponent<NewFighterOld>();
            if (owner != this)
            {
                actionHasHit = true;
            }
    ...
    public void GetThrown(Transform opponent, ActionData throwAction) 
    {
        ...
        throwOpponent = opponent.GetComponent<NewFighterOld>();
        SwitchState(new ThrowingOld(this));
