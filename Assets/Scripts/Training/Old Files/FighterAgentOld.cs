...
public class FighterAgentOld : Agent
{
    [Header("References")]
    public NewFighterOld self;
    public NewFighterOld opponent;
    ...
    public FighterCommandBufferOld commandBuffer;
    ...
    {
        if (self == null) self = GetComponent<NewFighterOld>();
        _timeSinceDecision = 0f;
        if (commandBuffer == null) commandBuffer = GetComponent<FighterCommandBufferOld>();
    }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Cooldown / lockout
        sensor.AddObservation(self.currentState is WalkingOld ? 1f : 0f);
        sensor.AddObservation(opponent.currentState is WalkingOld ? 1f : 0f);
    ...
    private float GetActionLockout01(NewFighterOld f)
    {
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        int attack = actions.DiscreteActions[2];

        commandBuffer?.ApplyDiscrete(move, jump, attack);
        ...
        if (self != null && opponent != null)
        {
            ...
            if (attack != 0 && normDist <= attackAttemptMaxNormDist)
            {
